...
## Features
- Password-protected upload/delete access saved in a long-lived browser session.
- Download/view files from `/files/<filename>` links without logging in.
- Delete files you no longer need from the web UI after login.
...
export QUICKDROP_PASSWORD=change-this-upload-password
...
```bash
sudo dnf install -y git python3 python3-pip policycoreutils-python-utils firewalld
...
Environment="QUICKDROP_PASSWORD=CHANGE_ME_TO_A_SHARED_UPLOAD_PASSWORD"
...
Now set a real secret in that service file and replace `CHANGE_ME_TO_A_SHARED_UPLOAD_PASSWORD` with your own shared upload password:
```bash
...
- Upload/delete access now uses a shared password from `QUICKDROP_PASSWORD` and stores the signed login session in the browser for `QUICKDROP_LOGIN_DAYS` days (default `30`).
...
- Downloads are still intentionally public. Anyone who can reach the app can fetch any file unless you also protect the site with firewall rules, VPN, or reverse-proxy auth.
